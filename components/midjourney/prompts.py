...
class ItemPrompt(MidJourneyPrompt):
    ...
    def create(self):
        ...
        # Else render 8K octane render
            ...
            prompt = f"{self.keyword}, {self.descriptors}, {self.asset_type}, photorealistic"
...
class TileMapPrompt(MidJourneyPrompt):
    ...
    def create(self):
        # Example: "A vast village scape game asset tile sheet of a farm, style of zelda a link to the past, no outlines, smooth pixel, pixel art, rpg maker, deviantart, artstation, itch.io, 32 by 32, tilemap --v 4"
        prompt = f"A vast village scape game asset tile sheet of a {self.keyword}, style of {self.style}, no outlines, smooth pixel, pixel art, rpg maker, deviantart, artstation, itch.io, 32 by 32, tilemap --v 4"
        self.write_prompt_to_file(prompt)
        return prompt
...
class TexturePrompt(MidJourneyPrompt):
    def __init__(self, colour,  keyword, type, style):
        super().__init__(keyword)
        self.colour = colour
        self.keyword = keyword
        self.type = type
    ...
    def create(self):
        ...
        prompt = f"minimalistic seamless {self.colour} {self.keyword} with {self.type} texture pattern, {self.style} style --v 4"
        self.write_prompt_to_file(prompt)
        return prompt
